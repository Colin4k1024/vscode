...
- An **authenticated** dev profile to seed from. By default the launcher derives the source from the product identity's `dataFolderName` (`product.json` overlaid with the gitignored `product.overrides.json` that `scripts/apply-mixin.mjs` writes) plus `-dev`: `~/.vscode-oss-dev` on a plain upstream checkout, `~/.open-agents-dev` on a codex-desktop branded one (macOS/Linux; `$env:USERPROFILE\<...>-dev` on Windows). This is the user-data-dir the repo's `launch.json` configs use - if the user has ever signed in to Copilot in a dev build of that identity, this should work. Only pass `--source-user-data-dir <path>` (or set `$CODE_OSS_DEV_AUTHED_USER_DATA_DIR`) when you specifically want to seed from a different profile (e.g. your regular `~/Library/Application Support/Code` install, or the pre-rebrand `~/.vscode-oss-dev` right after applying the D06 mixin).
