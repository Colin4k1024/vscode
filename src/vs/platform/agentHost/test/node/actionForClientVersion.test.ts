...
suite('actionForClientVersion', () => {
	...
	test('max known action version never exceeds the protocol version', () => {
		const max = Math.max(...Object.values(ACTION_INTRODUCED_IN).map(v => v.split('.').map(Number).reduce((a, b) => a * 1000 + (b ?? 0), 0)));
		const cur = PROTOCOL_VERSION.split('.').map(Number).reduce((a, b) => a * 1000 + (b ?? 0), 0);
