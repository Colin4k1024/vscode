...
import {
	...
	type IAgentHostOctoKitService,
} from "../../node/shared/agentHostOctoKitService.js";
import {
	type ICopilotApiService,
	type ICopilotApiServiceRequestOptions,
	type ICopilotUtilityChatCompletionRequest,
...
} from "../../node/shared/persistSessionMetadata.js";
...
class TestCopilotApiService implements ICopilotApiService {
	...
	messages(
		_githubToken: string,
		_request: Anthropic.MessageCreateParamsStreaming,
		_options?: ICopilotApiServiceRequestOptions,
	): AsyncGenerator<Anthropic.MessageStreamEvent>;
	messages(
		_githubToken: string,
		_request: Anthropic.MessageCreateParamsNonStreaming,
		_options?: ICopilotApiServiceRequestOptions,
...
suite("AgentHostSessionTitleController", () => {
	...
	function setup(
		...
		disposables.add(
			stateManager.onDidEmitEnvelope((e) => {
		...
		const controller = disposables.add(
			new AgentHostSessionTitleController(
				stateManager,
				{
					sessionDataService: createSessionDataService(db),
					queueCatalogSync: (session, metadataOverrides) =>
						catalogSyncs.push({ session, metadataOverrides }),
				...
				new NullLogService(),
				{
					...
					version: "1.0.0-test",
					excludeCopilotFromPackaging: false,
		...
		assert.deepStrictEqual(catalogSyncs, [
			{
				...
				metadataOverrides: {
					...
					[customChatTitleSourceMetadataKey(chat)]:
						AGENT_HOST_TITLE_SOURCE_AUTO,
				},
			},
			{
	...
	test("deferred mode persists its seed without utility requests or foreground naming instructions", async () => {
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"Fix https://github.com/microsoft/vscode/issues/123",
		);
		...
		assert.deepStrictEqual(
			{
				...
				instruction: await controller.prepareInstructionForAgent(
					session.toString(),
	...
	test("deferred refinement is nonblocking and starts only once", async () => {
		const { controller, stateManager, session, copilotApiService, db } =
			setupDeferred();
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn("Add dark mode", [textPart("Implemented dark mode.")]),
		]);
		...
		await waitForCondition(
			() => copilotApiService.utilityCalls.length === 1,
			"refinement should start",
		);
		assert.deepStrictEqual(
			{
				result,
				...
				turnState: stateManager.getSessionState(session.toString())?.turns[0]
					.state,
		...
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Dark mode setting",
			"refinement should persist",
		);
		controller.refineTitleFromFirstTurn(session.toString());
	...
	for (const { rename, generated } of [
		...
		test(`pending deferred refinement follows the default chat when a peer is added (rename: ${rename}, generated: ${generated})`, async () => {
					...
					stateManager,
					{
						sessionDataService: {
							...
							openDatabase: (resource) =>
								(resource.toString() === session.toString()
									? sessionData
									: chatData
								).openDatabase(resource),
			...
			stateManager.addChat(
				session.toString(),
				buildChatUri(session.toString(), "peer"),
				{},
			);
			if (rename !== undefined) {
			...
			assert.deepStrictEqual(
				{
					...
					persistedChatTitle: await db.getMetadata(
						customChatTitleMetadataKey(defaultChat),
					),
	...
	for (const initial of ["activeAgent", "utility", "deferred"] as const) {
		test(`snapshots ${initial} before session state exists and persists only after registration`, async () => {
			...
			const first = controller.getAutomaticTitleGenerationStrategy(
				session.toString(),
			);
			const beforeRegistration = await db.getMetadata(
				"titleGenerationStrategy",
			);
			activeAgent = !activeAgent;
			deferred = !deferred;
			const duringCreation = controller.getAutomaticTitleGenerationStrategy(
				session.toString(),
			);
			stateManager.createSession(createSummary(session));
			const registered = controller.getAutomaticTitleGenerationStrategy(
				session.toString(),
			);
			assert.deepStrictEqual(
				{
	...
	test("clears an unregistered strategy snapshot after failed creation", () => {
		...
		assert.deepStrictEqual(
			{
				first,
				retry: controller.getAutomaticTitleGenerationStrategy(
					session.toString(),
				),
			},
			{ first: "deferred", retry: "utility" },
		);
	});
	...
	for (const state of [TurnState.Cancelled, TurnState.Error]) {
		test(`deferred mode keeps its seed after ${state}`, async () => {
			...
			stateManager.seedDefaultChatTurns(session.toString(), [
				{
					...firstTurn("Add dark mode", [textPart("Partial response")]),
					state,
				},
			]);
			controller.refineTitleFromFirstTurn(session.toString());
	...
	test("deferred mode retains the fallback with no response text or credentials", async () => {
		for (const hasText of [false, true]) {
			const { controller, stateManager, session, copilotApiService, db } =
				setupDeferred(() => (hasText ? "" : "gh-token"));
			controller.seedTitleFromFirstMessage(session.toString(), "Add dark mode");
			stateManager.seedDefaultChatTurns(session.toString(), [
				firstTurn("Add dark mode", [textPart(hasText ? "Done" : "  \n")]),
			]);
			controller.refineTitleFromFirstTurn(session.toString());
			assert.deepStrictEqual(
				{
	...
	test("deferred utility failure keeps the persisted fallback and is not retried", async () => {
		...
		await waitForCondition(
			() => copilotApiService.utilityCalls.length === 1,
			"refinement should be attempted",
		);
		controller.refineTitleFromFirstTurn(session.toString());
		assert.deepStrictEqual(
			{
	...
	for (const phase of ["before", "during", "after"] as const) {
		test(`manual rename wins ${phase} deferred refinement even when it matches the seed`, async () => {
			const { controller, stateManager, session, copilotApiService } =
				setupDeferred();
			const pendingTitle = new DeferredPromise<string>();
			copilotApiService.responsePromise = pendingTitle.p;
			controller.seedTitleFromFirstMessage(session.toString(), "Add dark mode");
			...
			if (phase === "after") {
				await waitForCondition(
					() =>
						stateManager.getSessionState(session.toString())?.title ===
						"Generated title",
			...
			assert.deepStrictEqual(
				{
					...
					abortedDuring:
						phase === "during"
							? copilotApiService.utilityCalls[0]?.options?.signal?.aborted
							: undefined,
				},
				{
					title: "Add dark mode",
					calls: phase === "before" ? 0 : 1,
					abortedDuring: phase === "during" ? true : undefined,
				},
			);
		});
	}
	...
	for (const target of ["peer", "default", "defaultBeforePeer"] as const) {
		test(`deferred refinement targets the seeded ${target} title in multi-chat sessions`, async () => {
			...
			await waitForCondition(
				async () => (await db.getMetadata(key)) === "Generated title",
				"correct title should persist",
			);
			assert.deepStrictEqual(
				{
				...
				{
					chatTitle: "Generated title",
					sessionTitle: target === "defaultBeforePeer" ? "Add dark mode" : "",
				},
			);
		});
	}
	...
	test("deferred forks wait for their first new successful response", async () => {
		const { controller, stateManager, session, copilotApiService, db } =
			setupDeferred();
		const inherited = firstTurn("Original request", [
		...
		controller.generateForkedTitle(
			session.toString(),
			undefined,
			[inherited],
			"Forked: Original",
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			inherited,
			{ ...firstTurn("Fix dark mode", [textPart("Fixed")]), id: "turn-2" },
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Generated title",
			"fork should refine after its own response",
		);
	});
	...
	test("deferred seed following a local command refines the first provider response", async () => {
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			localTurn,
			{ ...firstTurn("Add dark mode", [textPart("Done")]), id: "turn-2" },
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Generated title",
			"first real response should refine",
		);
	});

	test("restored deferred auto seed refines only after its first successful response", async () => {
		const { controller, stateManager, session, db, copilotApiService } =
			setupDeferred();
		controller.seedTitleFromFirstMessage(session.toString(), "Add dark mode");
		const restored = disposables.add(
			new AgentHostSessionTitleController(
				stateManager,
				{
					sessionDataService: createSessionDataService(db),
		...
		await restored.restoreTitleGenerationStrategy(
			session.toString(),
			buildDefaultChatUri(session),
		);
		assert.deepStrictEqual(
			{
				strategy: restored.getAutomaticTitleGenerationStrategy(
					session.toString(),
				),
				instruction: await restored.prepareInstructionForAgent(
					session.toString(),
					buildDefaultChatUri(session),
				),
				calls: copilotApiService.utilityCalls.length,
	...
	for (const rawSeed of [
		...
		"[]",
		"{}",
		'{"title":42,"turnIndex":0}',
		'{"title":"Add dark mode","turnIndex":"0"}',
		'{"title":"Add dark mode","turnIndex":-1}',
		'{"title":"Add dark mode","turnIndex":0.5}',
	]) {
			...
			stateManager.seedDefaultChatTurns(session.toString(), [
				{
					...firstTurn(
						"Add dark mode",
						outcome === "empty" ? [] : [textPart("Done")],
					),
					state: outcome === "failed" ? TurnState.Error : TurnState.Complete,
				},
			]);
			controller.refineTitleFromFirstTurn(
				session.toString(),
				undefined,
				outcome !== "failed",
			);
			await waitForCondition(
				async () => (await db.getMetadata("deferredTitleSeed")) === "",
				"terminal outcome should consume eligibility",
			);
			if (outcome === "refined") {
				await waitForCondition(
					() => copilotApiService.utilityCalls.length === 1,
					"first refinement should run",
				);
			}
	...
	test("legacy restored sessions do not opt into deferred naming", async () => {
		...
		assert.strictEqual(
			controller.getAutomaticTitleGenerationStrategy(session.toString()),
			"activeAgent",
		);
	});
	...
	test("deferred disposal cancels pending work and ephemeral sessions do not seed", async () => {
		...
		ephemeral.controller.seedTitleFromFirstMessage(
			ephemeral.session.toString(),
			"Throwaway",
		);
	...
	test("active-agent mode completes the word crossing the 40-character fallback target without utility generation", async () => {
		...
		const { controller, session, db, titleActions } = setup(
			copilotApiService,
			"",
			undefined,
			undefined,
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"Investigate why restored Agent Host sessions sometimes lose titles",
		);
		const instruction = await controller.prepareInstructionForAgent(
			session.toString(),
			buildDefaultChatUri(session),
		);

		assert.deepStrictEqual(titleActions, [
			"Investigate why restored Agent Host sessions...",
		]);
		assert.strictEqual(copilotApiService.utilityCalls.length, 0);
		assert.strictEqual(
			instruction,
		...
		await waitForCondition(
			async () =>
				(await db.getMetadata(SESSION_CUSTOM_TITLE_SOURCE_KEY)) ===
				AGENT_HOST_TITLE_SOURCE_AUTO,
	...
	test("active-agent fallback hard-truncates a single oversized word", () => {
		const { controller, session, titleActions } = setup(
			undefined,
			"",
			undefined,
			undefined,
	...
	test("active-agent fallback hard-caps an oversized token crossing the target", () => {
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			`Fix https://example.com/${"x".repeat(500)}`,
		);
	...
	test("utility-model mode does not add an active-agent reminder", async () => {
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"Explain title generation",
		);

		assert.strictEqual(
			await controller.prepareInstructionForAgent(
				session.toString(),
				buildDefaultChatUri(session),
			),
			undefined,
	...
	test("does not generate or instruct titles for ephemeral sessions", async () => {
		const { controller, session, titleActions, copilotApiService } = setup(
			undefined,
			"",
			undefined,
			undefined,
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"Optimize an inline edit",
		);
		controller.seedProvisionalTitle(
			session.toString(),
			"Provisional inline edit",
		);
		controller.refineTitleFromFirstTurn(session.toString());
		controller.generateForkedTitle(
			session.toString(),
	...
	test("materialized server tools override later root setting changes", async () => {
		...
		enabled.controller.seedTitleFromFirstMessage(
			enabled.session.toString(),
			"Use advertised rename tool",
		);
		...
		disabled.controller.seedTitleFromFirstMessage(
			disabled.session.toString(),
			"Do not use missing rename tool",
		);

		assert.ok(
			(
				await enabled.controller.prepareInstructionForAgent(
					enabled.session.toString(),
					buildDefaultChatUri(enabled.session),
				)
			)?.includes("`rename_chat`"),
		);
		assert.strictEqual(
			await disabled.controller.prepareInstructionForAgent(
				disabled.session.toString(),
				buildDefaultChatUri(disabled.session),
			),
			undefined,
		);
	...
	test("active-agent mode reminds peer chats and keeps deterministic fork provenance without utility calls", async () => {
		...
		await waitForCondition(
			async () =>
				(await db.getMetadata(SESSION_CUSTOM_TITLE_SOURCE_KEY)) ===
				AGENT_HOST_TITLE_SOURCE_AUTO,
			"fork auto provenance should be persisted",
		);
		await waitForCondition(
			async () =>
				(await db.getMetadata(customChatTitleSourceMetadataKey(chat))) ===
				AGENT_HOST_TITLE_SOURCE_AUTO,
			"peer auto provenance should be persisted",
		);
	});

	test("multi-chat default uses its own persisted title provenance after controller recreation", async () => {
		const independentlyRenamed = setup(
			undefined,
			"Session title",
			undefined,
			undefined,
		...
		independentlyRenamed.stateManager.addChat(
			independentlyRenamed.session.toString(),
			buildChatUri(independentlyRenamed.session.toString(), "peer"),
			{},
		);
		await independentlyRenamed.db.setMetadata(
			SESSION_CUSTOM_TITLE_SOURCE_KEY,
			AGENT_HOST_TITLE_SOURCE_AUTO,
		);
		await independentlyRenamed.db.setMetadata(
			customChatTitleSourceMetadataKey(defaultChat),
			AGENT_HOST_TITLE_SOURCE_AGENT,
		);

		const independentRenameInstruction =
			await independentlyRenamed.controller.prepareInstructionForAgent(
				independentlyRenamed.session.toString(),
				defaultChat,
			);

		const independentlyAutomatic = setup(
			undefined,
			"Session title",
			undefined,
			undefined,
		...
		independentlyAutomatic.stateManager.addChat(
			independentlyAutomatic.session.toString(),
			buildChatUri(independentlyAutomatic.session.toString(), "peer"),
			{},
		);
		await independentlyAutomatic.db.setMetadata(
			SESSION_CUSTOM_TITLE_SOURCE_KEY,
			AGENT_HOST_TITLE_SOURCE_AGENT,
		);
		await independentlyAutomatic.db.setMetadata(
		...
		const independentAutoInstruction =
			await independentlyAutomatic.controller.prepareInstructionForAgent(
				independentlyAutomatic.session.toString(),
				defaultChat,
			);
	...
	test("clearSession releases session and peer-chat rename state", async () => {
		...
		assert.strictEqual(
			await controller.prepareInstructionForAgent(session.toString(), chat),
			undefined,
		);
	...
	test("clearSession cancels generation and clears every title-state collection", () => {
		...
		const provisionalChat = buildChatUri(
			session.toString(),
			"peer-provisional",
		);
	...
	test("seedTitleFromFirstMessage applies fallback and persists generated title", async () => {
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"  Please   explain title generation  ",
		);
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Generated title",
			"generated title should be persisted",
		);
		...
		assert.deepStrictEqual(
			{
				...
				promptIncludesUserText:
					copilotApiService.utilityCalls[0]?.request.messages.some((message) =>
						message.content.includes("Please   explain title generation"),
					),
	...
	test("seedTitleFromFirstMessage appends every unique GitHub issue and pull request", async () => {
		...
		octoKitService.responses.set("microsoft/vscode#123", {
			title: "Issue title",
			body: "Issue body",
		});
		octoKitService.responses.set("microsoft/vscode#456", {
			title: "Pull request title",
			body: "Pull request body",
		});
		const { controller, session, db } = setup(
			copilotApiService,
			"",
			() => "gh-token",
			octoKitService,
		...
		const prompt =
			"Fix https://github.com/microsoft/vscode/issues/123 and review https://github.com/microsoft/vscode/pull/456. Duplicate: https://www.github.com/microsoft/vscode/issues/123#issuecomment-1";
		...
		const userMessage = copilotApiService.utilityCalls[0].request.messages.find(
			(message) => message.role === "user",
		)?.content;
		assert.deepStrictEqual(
			{
				calls: octoKitService.calls.map((call) => ({
					owner: call.owner,
					repo: call.repo,
					number: call.number,
					token: call.token,
	...
	test("seedTitleFromFirstMessage only fetches links from the configured GitHub host", async () => {
		...
		const prompt =
			"Compare https://github.com/microsoft/vscode/issues/123 with https://github.enterprise.test/microsoft/vscode/issues/456";
		...
		const userMessage =
			copilotApiService.utilityCalls[0].request.messages.find(
				(message) => message.role === "user",
			)?.content ?? "";
		assert.deepStrictEqual(
			{
				...
				hasEnterpriseIssue: userMessage.includes(
					"The title of the issue is: Enterprise issue",
				),
			},
			{
				calls: [456],
	...
	test("seedTitleFromFirstMessage fetches at most ten GitHub references", async () => {
		...
		for (let number = 1; number <= 11; number++) {
			octoKitService.responses.set(`microsoft/vscode#${number}`, {
				title: `Issue ${number}`,
				body: `Body ${number}`,
			});
		...
		assert.deepStrictEqual(
			{
				calls: octoKitService.calls.map((call) => call.number),
				hasTenthContext: userMessage.includes(
					"The title of the issue is: Issue 10",
				),
				hasEleventhContext: userMessage.includes(
					"The title of the issue is: Issue 11",
				),
			},
			{
	...
	test("seedTitleFromFirstMessage keeps successful GitHub context when another request fails", async () => {
		...
		const prompt =
			"Fix https://github.com/microsoft/vscode/issues/123 and https://github.com/microsoft/vscode/pull/456";
		...
		assert.deepStrictEqual(
			{
				hasIssue: userMessage.includes(
					"The title of the issue is: Issue title",
				),
				hasPullRequest: userMessage.includes(
					"GitHub pull request microsoft/vscode#456",
				),
			},
			{
	...
	test("seedTitleFromFirstMessage times out GitHub context requests", async () => {
		...
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Generated title",
			"generated title should be persisted after the GitHub request times out",
		);
	...
	test("seedTitleFromFirstMessage caps each appended GitHub body at 4000 characters", async () => {
		...
		const context = userMessage.slice(
			userMessage.indexOf("GitHub issue and pull request context:"),
		);
	...
	test("seedTitleFromFirstMessage caps the combined prompt and GitHub context", async () => {
		...
		const octoKitService = new TestAgentHostOctoKitService();
		octoKitService.responses.set("microsoft/vscode#123", {
			title: `start${"x".repeat(30_000)}end`,
			body: "",
		});
		const { controller, session, db } = setup(
			copilotApiService,
	...
	test("seedTitleFromFirstMessage strips an unexpected trailing Han suffix from a Latin title", async () => {
		...
		const cases = [
			...
			{
				response: "Fix chat title \u7f16\u7801\u95ee",
				expected: "Fix chat title",
			},
			{
				response: `${titlePrefixAtLimit}\u7f16\u7801`,
				expected: titlePrefixAtLimit,
			},
		];
		...
		for (const testCase of cases) {
			...
			const { controller, stateManager, session, db } =
				setup(copilotApiService);
		...
		assert.deepStrictEqual(
			...
			cases.map((testCase) => ({
				...
				persistedTitle: testCase.expected,
			})),
		);
	});
	...
	test("seedTitleFromFirstMessage preserves intentional or ambiguous Han suffixes", async () => {
		const cases = [
			{
				prompt: "Explain \u7f16\u7801 naming",
				response: "Explain code\u7f16\u7801",
			},
			{ prompt: "Fix chat title generation", response: "Fix chat title\u7f16" },
			{
				prompt: "Fix chat title generation",
				response: "Fix chat title\u7f16\u7801\u95ee\u9898",
			},
			{
				prompt: "Fix chat title generation",
				response: "\u4fee\u590d\u6807\u9898",
		...
		assert.deepStrictEqual(
			titles,
			cases.map((testCase) => ({
				title: testCase.response,
				persistedTitle: testCase.response,
			})),
		);
	});
	...
	test("cancelTitleGeneration cancels delayed generated title application", async () => {
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"Investigate title cancellation",
		);
		await waitForCondition(
			() => copilotApiService.utilityCalls.length === 1,
			"title generation should start",
	...
	test("seedProvisionalTitle titles the session from the suggestion without generating", async () => {
		const copilotApiService = new TestCopilotApiService();
		const { controller, stateManager, session, db, titleActions } =
			setup(copilotApiService);

		controller.seedProvisionalTitle(session.toString(), "ls -la");
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "ls -la",
			"provisional title should be persisted",
		);
	...
	test("seedProvisionalTitle refreshes a provisional title with a later suggestion", async () => {
		...
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "ls -la",
			"first provisional title should be persisted",
		);
		controller.seedProvisionalTitle(session.toString(), "git status");
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "git status",
			"second provisional title should be persisted",
		);
	...
	test("seedTitleFromFirstMessage replaces a provisional title with a generated title", async () => {
		...
		controller.seedTitleFromFirstMessage(
			session.toString(),
			"Explain how the build works",
		);
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Explain the build",
			"generated title should replace the provisional title",
		);
	...
	test("seedTitleFromFirstMessage persists its fallback when replacing a provisional title", async () => {
		...
		await waitForCondition(
			async () =>
				(await db.getMetadata("customTitle")) === "Explain how the build works",
			"fallback title should replace the provisional title",
		);
	...
	async function seedFirstTitle(
		...
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === title,
			"first title should be persisted",
		);
	}
	...
	test("refineTitleFromFirstTurn regenerates the title from the first-turn context", async () => {
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn("Add dark mode toggle", [
				textPart("Implemented the toggle in the settings editor."),
			]),
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Dark mode setting",
			"refined title should be persisted",
		);

		const lastCall =
			copilotApiService.utilityCalls[copilotApiService.utilityCalls.length - 1];
		const userMessage =
			lastCall.request.messages.find((message) => message.role === "user")
				?.content ?? "";
		assert.deepStrictEqual(
			{
				title: stateManager.getSessionState(session.toString())?.title,
				...
				includesResponse: userMessage.includes(
					"Implemented the toggle in the settings editor.",
				),
			},
			{
	...
	test("refineTitleFromFirstTurn does not clobber a title changed in the meantime", async () => {
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn("Add dark mode toggle", [textPart("Implemented the toggle.")]),
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await Promise.resolve();
	...
	test("refineTitleFromFirstTurn ignores tool calls and reasoning, keeping only text parts", async () => {
		...
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			() => copilotApiService.utilityCalls.length >= 2,
			"refine should issue a utility call",
		);
		...
		assert.deepStrictEqual(
			{
				...
				excludesToolCall:
					!userMessage.includes("TOOL_MARKER") &&
					!userMessage.includes("SearchTool"),
			},
			{
				includesText: true,
	...
	test("refineTitleFromFirstTurn truncates the middle of an oversized text response", async () => {
		...
		copilotApiService.response = "Refined title";
		const hugeResponse =
			"A".repeat(15000) + " MIDDLE_MARKER " + "B".repeat(15000);
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn("Add dark mode toggle", [textPart(hugeResponse)]),
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			() => copilotApiService.utilityCalls.length >= 2,
		...
		assert.deepStrictEqual(
			{
				...
				middleTruncated:
					userMessage.includes("...") && !userMessage.includes("MIDDLE_MARKER"),
				includesUserRequest: userMessage.includes("Add dark mode toggle"),
				keepsHeadAndTail:
					userMessage.includes("AAAA") && userMessage.includes("BBBB"),
			},
			{
				withinBudget: true,
	...
	test("refineTitleFromFirstTurn appends GitHub context from the request and offers the current title", async () => {
		...
		const octoKitService = new TestAgentHostOctoKitService();
		octoKitService.responses.set("microsoft/vscode#123", {
			title: "Agent Host logs an error when a local commit is not on GitHub",
			body: "Issue body",
		});
		const { controller, stateManager, session, db } = setup(
			copilotApiService,
		...
		const request =
			"Tackle this issue: https://github.com/microsoft/vscode/issues/123";
		await seedFirstTitle(
			controller,
			copilotApiService,
			db,
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn(request, [textPart("Fixed the pull request lookup.")]),
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			async () =>
				(await db.getMetadata("customTitle")) === "Missing commit lookup error",
			"refined title should be persisted",
		);
		...
		assert.deepStrictEqual(
			{
				...
				includesIssueTitle: userMessage.includes(
					"The title of the issue is: Agent Host logs an error when a local commit is not on GitHub",
				),
				includesResponse: userMessage.includes(
					"Fixed the pull request lookup.",
				),
				includesCurrentTitle: userMessage.includes(
					"Its current title is: First title",
				),
			},
			{
				fetched: [123, 123],
	...
	test("refineTitleFromFirstTurn ignores GitHub links the agent only mentioned in its response", async () => {
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn(request, [
				textPart(
					"This also affects https://github.com/microsoft/vscode/issues/456",
				),
			]),
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Refined title",
			"refined title should be persisted",
		);
		...
		assert.deepStrictEqual(
			{
				fetched: octoKitService.calls.map((call) => call.number),
				includesMentionedIssueContext: userMessage.includes(
					"The title of the issue is: Mentioned issue",
				),
			},
			{
				fetched: [123, 123],
	...
	test("refineTitleFromFirstTurn keeps the issue title within budget despite an oversized response", async () => {
		...
		const octoKitService = new TestAgentHostOctoKitService();
		octoKitService.responses.set("microsoft/vscode#123", {
			title: "Local commit lookup fails",
		...
		copilotApiService.response = "Refined title";
		const hugeResponse =
			"A".repeat(15_000) + " MIDDLE_MARKER " + "B".repeat(15_000);
		stateManager.seedDefaultChatTurns(session.toString(), [
			firstTurn(request, [textPart(hugeResponse)]),
		]);
		controller.refineTitleFromFirstTurn(session.toString());
		await waitForCondition(
			async () => (await db.getMetadata("customTitle")) === "Refined title",
		...
		const userMessage =
			lastCall.request.messages.find((message) => message.role === "user")
				?.content ?? "";
		const promptContent = userMessage.slice(
		...
		assert.deepStrictEqual(
			{
				...
				includesIssueTitle: promptContent.includes(
					"The title of the issue is: Local commit lookup fails",
				),
				keepsResponseHeadAndTail:
					promptContent.includes("AAAA") && promptContent.includes("BBBB"),
	...
	test("generateForkedTitle replaces the inherited title using the whole forked conversation", async () => {
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			turn("turn-1", "Add dark mode toggle", [
				textPart("Implemented the toggle in settings."),
			]),
			turn("turn-2", "Now compact the history", [
				textPart("Summarized earlier turns."),
			]),
		]);
		...
		await waitForCondition(
			async () =>
				(await db.getMetadata("customTitle")) === "Compaction strategy",
			"forked title should be persisted",
		);
		...
		assert.deepStrictEqual(
			{
				...
				persistedTitle: await db.getMetadata("customTitle"),
				mentionsConversation: userMessage.includes("conversation"),
				framesAsBranch: userMessage.includes(
					'branched from an earlier chat titled "Source title"',
				),
				includesFirstTurn:
					userMessage.includes("Add dark mode toggle") &&
					userMessage.includes("Implemented the toggle in settings."),
				includesSecondTurn:
					userMessage.includes("Now compact the history") &&
					userMessage.includes("Summarized earlier turns."),
			},
			{
	...
	test("generateForkedTitle does not clobber a title changed during generation", async () => {
		...
		stateManager.seedDefaultChatTurns(session.toString(), [
			turn("turn-1", "Add dark mode toggle", [textPart("Done.")]),
		]);
		controller.generateForkedTitle(
			session.toString(),
			undefined,
			stateManager.getSessionState(session.toString())!.turns,
			"Forked: Source title",
		);
		await waitForCondition(
			() => copilotApiService.utilityCalls.length === 1,
			"forked title generation should start",
		);
		stateManager.dispatchServerAction(session.toString(), {
			type: ActionType.SessionTitleChanged,
			title: "Manual title",
	...
	test("generateExternalSessionTitle titles a surfaced external session from its first prompt", async () => {
		...
		disposables.add(
			stateManager.onDidEmitNotification((n) => {
				if (
					n.type === NotificationType.SessionSummaryChanged &&
					n.session === external.toString()
				) {
		...
		await controller.generateExternalSessionTitle(
			external.toString(),
	...
	test("generateExternalSessionTitle does not clobber a rename during generation", async () => {
		...
		stateManager.announceSurfacedSession(createSummary(external));
		const generation = controller.generateExternalSessionTitle(
			external.toString(),
			"Fix the flaky renderer test",
		);
		await waitForCondition(
