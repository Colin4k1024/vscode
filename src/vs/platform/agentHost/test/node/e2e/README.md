...
- **WebSocket transport probes are not model traffic**: the Codex `openai` provider (used by the OpenAI-credential scenarios) probes `GET /responses` with an `Upgrade: websocket` header before streaming over plain HTTP. The proxy answers these probes `426 Upgrade Required` — never a cache miss — which makes the provider fall back to the recordable SSE transport immediately. Strictness is unaffected: a provider that switched to websocket-only responses would leave the recorded POST exchanges unconsumed and fail teardown.
