...
| B17 | Elicitation with unknown semantic input | **covered by unit test (mapping) + live-only (end-to-end)** | The deterministic contract lives in `codexElicitationMapper` and is pinned by `test/node/codex/codexElicitationMapper.test.ts`: `mode: "openai/form"` (or any unprojectable mode) surfaces a **message-only** `ChatInputRequest` (no half-rendered questions), and decline/cancel map to the MCP `decline`/`cancel` actions — never a JSON-RPC error. The end-to-end elicitation round trip is registered live-only below: a codex MCP elicitation requires the model to call an MCP tool, and the replayed tool call reaches the codex tool router before the server's post-handshake inventory refresh registers the tool (verified: router rejects with `unsupported call` even with the server `ready`). The generic elicitation pathway also has deterministic replay coverage on the Copilot provider (`captures/copilotcli-plugin-mcp-form-elicitation-*.yaml`). |
...
| B17 end-to-end MCP elicitation (unknown or any mode) | Reaching a codex MCP elicitation requires the model to call the server's tool. Codex only registers an MCP server's tools with its tool router after the post-handshake inventory refresh; the replayed model response arrives ~10ms after `ready` and is rejected (`unsupported call`). A live model's natural latency lets the call land. The mapping contract itself is deterministic and unit-covered (see the B17 row). |
