	...
	private renderWelcomeViewContentIfNeeded() {
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.completed && product.defaultChatAgent?.provider.default && product.defaultChatAgent.termsStatementUrl && product.defaultChatAgent.privacyStatementUrl) {
