...
import { AgentHostCodexDefaultsContribution } from '../../../browser/agentSessions/agentHost/agentHostCodexDefaultsContribution.js';
...
const fullSchema: Record<string, ConfigPropertySchema> = {
	[AgentHostConfigKey.CodexPreferOpenAIProvider]: { type: 'boolean', title: 'Codex Prefers the OpenAI Provider by Default' },
};
...
function setup(disposables: DisposableStore, settings: Record<string, unknown>) {
	...
	disposables.add(instantiationService.createInstance(AgentHostCodexDefaultsContribution));
	return { agentHostService, configurationService };
}

suite('AgentHostCodexDefaultsContribution', () => {
	...
	test('forwards the policy flag into root config under the short key once the schema advertises it', async () => {
		const { agentHostService } = setup(disposables, {
			[AgentHostCodexPreferOpenAIProviderSettingId]: true,
		});
		...
		assert.deepStrictEqual((agentHostService.dispatchedActions[0].action as IRootConfigChangedAction).config, {
			[AgentHostConfigKey.CodexPreferOpenAIProvider]: true,
		});
	});
	...
	test('re-pushes the short key when the setting is toggled on', async () => {
		const { agentHostService, configurationService } = setup(disposables, {
			[AgentHostCodexPreferOpenAIProviderSettingId]: false,
		});
		agentHostService.setRootState(makeRootStateWithSchema(fullSchema));
		await flush();

		await configurationService.setUserConfiguration(AgentHostCodexPreferOpenAIProviderSettingId, true);
		fireConfigurationChange(configurationService, AgentHostCodexPreferOpenAIProviderSettingId);
		await flush();

		assert.deepStrictEqual(
			agentHostService.dispatchedActions.map(a => (a.action as IRootConfigChangedAction).config),
			[
				{ [AgentHostConfigKey.CodexPreferOpenAIProvider]: false },
				{ [AgentHostConfigKey.CodexPreferOpenAIProvider]: true },
			],
		);
	});
