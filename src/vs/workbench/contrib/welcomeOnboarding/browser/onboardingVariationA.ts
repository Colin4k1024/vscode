...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		// GitHub Copilot disclaimer — only rendered when the product actually
		// names a default chat agent. Without one (this product, post-D08) the
		// Copilot-branded text and its links have no target, so skip the block.
		if (defaultChat) {
		...
		const validate = (): boolean => {
			...
			switch (result.kind) {
				...
				case GheParseResultKind.Invalid:
					...
					message.textContent = localize('onboarding.signIn.enterprise.invalid', 'You must enter a valid {0} instance (i.e. "octocat" or "https://octocat.ghe.com")', defaultChat?.provider.enterprise.name ?? 'GitHub Enterprise');
					submitAction.enabled = false;
					return false;
			}
		};
	...
	private _renderEnterpriseSignInProgress(actions: HTMLElement): void {
		...
		message.textContent = localize('onboarding.signIn.enterprise.progress', "Waiting for {0} sign-in to complete...", defaultChat?.provider.enterprise.name ?? 'GitHub Enterprise');
	}

	private _getEnterpriseInstancePromptLabel(): string {
		return localize('onboarding.signIn.enterprise.prompt', "What is your {0} instance?", defaultChat?.provider.enterprise.name ?? 'GitHub Enterprise');
	}
	...
	private async _handleEnterpriseSignIn(): Promise<void> {
		const existingUri = this.configurationService.getValue<string>(defaultChat?.providerUriSetting ?? 'github-enterprise.uri');
	...
	private async _submitEnterpriseInstance(resolvedUri: string): Promise<void> {
		try {
			await this.configurationService.updateValue(defaultChat?.providerUriSetting ?? 'github-enterprise.uri', resolvedUri, ConfigurationTarget.USER);
	...
	private async _runEnterpriseSignInSetup(): Promise<void> {
		...
		const provider = defaultChat?.provider.enterprise.id ?? 'github-enterprise';
