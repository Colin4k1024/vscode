...
// The branded fork deletes product.defaultChatAgent (D08 mixin overlay), so a
// module-level assert here would throw at import time and take the whole
// workbench chunk down with it (blank window). Assert lazily instead: the
// wizard no-ops in products without a default chat agent (see show()).
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		if (!product.defaultChatAgent) {
			// No default chat agent configured (branded product, D08): the
			// sign-in-centric onboarding wizard does not apply — skip it.
			return;
		}
